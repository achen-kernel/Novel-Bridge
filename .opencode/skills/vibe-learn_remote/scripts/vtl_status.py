...
def main():
    ...
    stop_condition = None
    if state is None:
        warnings.append("Missing or invalid docs/learn_remote/vtl-state.json.")
        stop_condition = "create_vtl_state"
    if not stage_path.exists():
        warnings.append("Missing docs/learn_remote/current-stage.md.")
    ...
    next_actions = []
    if state is None:
        next_actions.append("Create docs/learn_remote/vtl-state.json or run scripts/vtl_init.py --json")
    else:
